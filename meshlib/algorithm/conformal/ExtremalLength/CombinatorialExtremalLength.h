/*!
*      \file CombinatorialExtremalLength.h
*      \brief Algorithm for compute combinaotiral harmonic 1-forms for topological quadralateral 
*	   \author David Gu
*      \date Document 10/26/2014
*
*		Computing extremal length
*
*/

#ifndef _COMBINATORIAL_EXTREMAL_LENGTH_H_
#define _COMBINATORIAL_EXTREMAL_LENGTH_H_
...
{
	/*! \brief CCombinatorialExtremalLength class
	*  
	*  Algorithm for computing combinatorial extremal length
	*/
  template<typename M>
  class CCombinatorialExtremalLength
  {
  public:
	  /*! CCombinatorialExtremalLength constructor
	  *
	  *	\param pMesh input mesh, topological disk
	  */
     CCombinatorialExtremalLength( M * pMesh );
	  /*! CCombinatorialExtremalLength destructor
	  */
    ~CCombinatorialExtremalLength();
...
template<typename M>
CCombinatorialExtremalLength<M>::CCombinatorialExtremalLength( M * pMesh ):m_pMesh( pMesh ), m_boundary( m_pMesh )
{

	_segment_boundary();

	MeshLib::COperator<M> pC( m_pMesh );
	pC._combinatorial_Laplace();

}

template<typename M>
CCombinatorialExtremalLength<M>::~CCombinatorialExtremalLength()
{
...
template<typename M>
void CCombinatorialExtremalLength<M>::calculate_harmonic_exact_form(const char * prefix )
{
...
template<typename M>
void CCombinatorialExtremalLength<M>::_harmonic_exact_form( std::vector<typename M::CVertex*>& top_segment,std::vector<typename M::CVertex*>& bottom_segment  )
{
...
template<typename M>
void CCombinatorialExtremalLength<M>::_segment_boundary()
{
...
#endif _COMBINATORIAL_EXTREMAL_LENGTH_H_