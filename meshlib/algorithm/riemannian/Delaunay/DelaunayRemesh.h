/*! \file DelaunayRemesh.h
*   \brief Algorithm: Remesh a given mesh using Ruppert's Delaunay refinement algorithm
...
#ifndef  _DELAUNAY_REMESH_H_
#define  _DELAUNAY_REMESH_H_
...
/*!	\brief CDelaunayRemeshVertex class
	 *  
	 ...
	 */
  class CDelaunayRemeshVertex : public  CVertex
  {
  protected:
    CPoint2  m_uv;
	CPoint   m_rgb;
...
/*! Read vertex uv, rgb traits from string */
 ...
 inline void CDelaunayRemeshVertex::_from_string()
 {
...
/*! write vertex uv, rgb traits to string */
  ...
  inline void CDelaunayRemeshVertex::_to_string()
  {
		  CParser parser( m_string );
  ...
  /*!	\brief CDelaunayRemeshEdge class
	 *  
 ...
 class CDelaunayRemeshEdge : public  CEdge
 {
 public:
	 CDelaunayRemeshEdge() { m_frozen = false; };
 ...
 inline void CDelaunayRemeshEdge::_to_string()
 {
	 if( m_frozen ) m_string="sharp";
 }
 /*!	\brief CDelaunayRemeshHalfEdge class
	 *  
 ...
 class CDelaunayRemeshHalfEdge : public CHalfEdge
 {
 public:
	 double & angle() { return m_angle; };
 public:
  ...
  /*!	\brief CDelaunayRemeshFace class
	 *  
	 *	Face class for Remeshing using Delaunay triangulation
	 *	Traits, area, uv_area, minimal angle, quality
	 */

class CDelaunayRemeshFace : public CFace
 {
 public:
	 
	 CDelaunayRemeshFace() { m_inside = true;  }; //by default all faces are inside
...
inline void CDelaunayRemeshFace::_to_string()
{
	if( !m_inside )
	{
		m_string = "rgb=(1 0 0)";
...
/*
CSample operator+( CSample & s1, CSample & s2 )
{
	CSample r;
	r.m_pos = s1.m_pos + s2.m_pos;
	r.m_rgb = s1.m_rgb + s2.m_rgb;
	r.m_lambda = s1.m_lambda + s2.m_lambda;
	r.m_k = s1.m_k + s2.m_k;
	return r;
};

CSample operator*( CSample & s1, double d )
{
	CSample r;
	r.m_pos = s1.m_pos * d;
	r.m_rgb = s1.m_rgb * d;
	r.m_lambda = s1.m_lambda * d;
	r.m_k = s1.m_k * d;
	return r;
};
*/

/*! \brief CDeluanayRemesh class
*   
*   Remesh a given surface using Ruppert's Delaunay refinement algorithm.
*/
template<typename V, typename E, typename F, typename H>
class CDelaunayRemesh : public CDelaunayMesh<V,E,F,H>
{
public:
	...
	*/
	void Remesh( CDelaunayRemesh<V,E,F,H> & input_mesh );

	/*! \brief Compute the conformal factor
	*/
	void _lambda();

	void Convert( CDelaunayRemesh<V,E,F,H> & output_mesh );
...
protected:
	...
	*/
	F * _locate_point(  CPoint2 & pt,  CSample & sample );
	/*!	background mesh to be sampled.
	 *
	 */
	CDelaunayRemesh<V,E,F,H> * m_background;
...
template<typename V, typename E, typename F, typename H>
void CDelaunayRemesh<V,E,F,H>::_lambda()
{
	double total_area = 0;
...
template<typename V, typename E, typename F, typename H>
void CDelaunayRemesh<V,E,F,H>::Remesh( CDelaunayRemesh<V,E,F,H> & mesh )
{
	int N = 256;
	double r = 0.98;
	//double r = 0.95;
...
template<typename V, typename E, typename F, typename H>
F* CDelaunayRemesh<V,E,F,H>::_locate_point(  CPoint2 & pt, CSample & sample )
{
...
template<typename V, typename E, typename F, typename H>
F* CDelaunayRemesh<V,E,F,H>::_locate_point(  CPoint2 & pt, CPoint & q, CPoint & rgb )
{
...
template<typename V, typename E, typename F, typename H>
bool CDelaunayRemesh<V,E,F,H>::_insert_vertex(CPoint2 & pt, V * & pV )
{
...
template<typename V, typename E, typename F, typename H>
V* CDelaunayRemesh<V,E,F,H>::_insert_vertex_on_edge( E * e, CPoint2 & pt )
{
...
template<typename V, typename E, typename F, typename H>
void CDelaunayRemesh<V,E,F,H>::_statistics( V* v0, V* v1, V* v2, double & minAngle, double & quality)
...
template<typename V, typename E, typename F, typename H>
void CDelaunayRemesh<V,E,F,H>::Convert( CDelaunayRemesh<V,E,F,H> & output_mesh )
{
	for( MeshVertexIterator viter( this ); !viter.end(); viter ++ )
	{
		V * pV = *viter;
...
typedef CDelaunayRemesh<CDelaunayRemeshVertex, CDelaunayRemeshEdge, CDelaunayRemeshFace, CDelaunayRemeshHalfEdge> CSDRMesh;
