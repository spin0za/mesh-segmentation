/*! \file QCInversiveDistanceRicciFlow.h
 * \brief Quasi-Conformal Euclidean inversive distance Ricci flow
 *  \author David Gu
 *  \date   documented on 03/09/2011
 *
 *	Algorithm for Quasi-Conformal Inversive distance  Ricci Flow
 */

#ifndef _QC_INVERSIVE_DISTANCE_RICCI_FLOW_H_
#define _QC_INVERSIVE_DISTANCE_RICCI_FLOW_H_
...
namespace RicciFlow
{
	/*! \brief QCCInversiveDistanceRicciFlow class
	 *  
	 *  Algorithm for Inversive distance Ricci flow
	 */
	 
 template<typename M>
 class CQCInversiveDistanceRicciFlow : public CBaseRicciFlow<M>
  {
  public:
	   ...
	   */
	  CQCInversiveDistanceRicciFlow( M * pMesh );
...
template<typename M>
CQCInversiveDistanceRicciFlow<M>::CQCInversiveDistanceRicciFlow( M * pMesh ): CBaseRicciFlow( pMesh)
{
	//set up Beltrami coefficients

	for( M::MeshVertexIterator viter( m_pMesh ); !viter.end(); viter ++ )
	{
		M::CVertex * v = *viter;
	  CPoint2 p = v->huv();
	  //std::complex<double> mu(p[0]/2.0,p[1]/2.0);
...
template<typename M>
void CQCInversiveDistanceRicciFlow<M>::_length( double u1, double u2, typename M::CEdge * e )
{
	  double r1 = exp( u1 );
	  double r2 = exp( u2 );
...
template<typename M>
void CQCInversiveDistanceRicciFlow<M>::_calculate_edge_weight()
{
...
template<typename M>
void CQCInversiveDistanceRicciFlow<M>::_calculate_metric()
{
...
template<typename M>
void CQCInversiveDistanceRicciFlow<M>::_calculate_initial_inversive_distance()
{
...
template<typename M>
void CQCInversiveDistanceRicciFlow<M>::_calculate_power_center( typename M::CFace * head )
{
...
template<typename M>
void CQCInversiveDistanceRicciFlow<M>::_calculate_edge_weight_by_power_center()
{
	for( M::MeshFaceIterator fiter( m_pMesh ); !fiter.end(); fiter ++ )
	{
		M::CFace * head = *fiter;
...
template<typename M>
double CQCInversiveDistanceRicciFlow<M>::_cosine_law( double a, double b, double c )
{
...
template<typename M>
void CQCInversiveDistanceRicciFlow<M>::_normalization( Eigen::VectorXd & x, int num )
{
...
template<typename M>
void CQCInversiveDistanceRicciFlow<M>::_set_target_curvature()
{
  for( M::MeshVertexIterator viter( m_pMesh ); !viter.end(); viter ++ )
  {
	  M::CVertex * v = *viter;
...
#endif  _QC_INVERSIVE_DISTANCE_RICCI_FLOW_H_