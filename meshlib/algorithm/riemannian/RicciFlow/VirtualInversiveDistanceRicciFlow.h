/*! \file VirtualInversiveDistanceRicciFlow.h
 * \brief Euclidean inversive distance Ricci flow with virtual radius
 *  \author David Gu
...
#ifndef _VIRTUAL_INVERSIVE_DISTANCE_RICCI_FLOW_H_
#define _VIRTUAL_INVERSIVE_DISTANCE_RICCI_FLOW_H_
...
namespace RicciFlow
{
 ...
 template<typename M>
 class CVirtualInversiveDistanceRicciFlow : public CBaseRicciFlow<M>
 {
  public:
	  /*! \brief CVirtualInversiveDistanceRicciFlow constructor
	   *
	   *  call base class constructor 
	   */
	  CVirtualInversiveDistanceRicciFlow( M * pMesh );
...
template<typename M>
CVirtualInversiveDistanceRicciFlow<M>::CVirtualInversiveDistanceRicciFlow( M * pMesh ): CBaseRicciFlow( pMesh)
{
	_normalize_mesh();
...
template<typename M>
void CVirtualInversiveDistanceRicciFlow<M>::_calculate_edge_weight()
{
	_calculate_edge_weight_by_power_center();
}

template<typename M>
void CVirtualInversiveDistanceRicciFlow<M>::_calculate_metric( double step_length )
{
...
template<typename M>
void CVirtualInversiveDistanceRicciFlow<M>::_calculate_initial_inversive_distance()
{
...
template<typename M>
void CVirtualInversiveDistanceRicciFlow<M>::_calculate_power_center( typename M::CFace * head )
{
...
template<typename M>
void CVirtualInversiveDistanceRicciFlow<M>::_calculate_edge_weight_by_power_center()
{
	for( M::MeshFaceIterator fiter( m_pMesh ); !fiter.end(); fiter ++ )
	{
		M::CFace * head = *fiter;
...
template<typename M>
double CVirtualInversiveDistanceRicciFlow<M>::_cosine_law( double a, double b, double c )
{
...
template<typename M>
void CVirtualInversiveDistanceRicciFlow<M>::_set_target_curvature()
{
  for( M::MeshVertexIterator viter( m_pMesh ); !viter.end(); viter ++ )
  {
	  M::CVertex * v = *viter;
...
template<typename M>
void CVirtualInversiveDistanceRicciFlow<M>::_normalize_mesh()
{
 ...
 //calculate edge length
template<typename M>
void CVirtualInversiveDistanceRicciFlow<M>::_length( double u1, double u2, typename M::CEdge * e )
{
	//double r1 = exp( u1 );
	//double r2 = exp( u2 );
...
#endif  _VIRTUAL_INVERSIVE_DISTANCE_RICCI_FLOW_H_